...
def find_movies_names(python_dictionary_values):
    ...
    profile_name = str(x['original_title']) + \
        ' (' + str(x['release_date'][:4]) + ')'
    if profile_path == None:
...
def search_person(mylist, mynames, search):
    ...
    for i in mylist:
        s = i.replace(' ', '%20')
        url = 'https://api.themoviedb.org/3/search/'+search+'?api_key=' + \
            api_key+'&language=en-US&query=%27'+s+'%27&page=1&include_adult=false'
...
def get_genre(idx):
    url = 'https://api.themoviedb.org/3/movie/' + \
        str(idx)+'?api_key='+api_key+'&language=en-US'
    response = requests.get(url)
    python_dictionary_values = json.loads(response.text)
...
def movie_details(movie):
    ...
    if mymovie['poster_path'] != None:
        profile_image.append(
            'https://image.tmdb.org/t/p/w185/' + mymovie['poster_path'])
    else:
        profile_image.append(
    ...
    if mymovie['backdrop_path'] != None:
        profile_image.append(
            'https://image.tmdb.org/t/p/w185/' + mymovie['backdrop_path'])
    ...
    url = 'https://api.themoviedb.org/3/movie/' + \
        str(idx)+'/credits?api_key='+api_key+'&language=en-US'
    response = requests.get(url)
    python_dictionary_values = json.loads(response.text)
    ...
    myactors = df[df['known_for_department']
                  == 'Acting'].original_name.tolist()
    mynames = df[df['known_for_department'] == 'Acting'].mynames.tolist()
    movie_cast, cast_url = search_person(
        myactors[:10], mynames[:10], search='person')
    x = myactors[:12]

    myactors = df2[df2['department'] == 'Directing'].original_name.tolist()
    movie_directors, directors_url = search_person(
        myactors[:4], myactors[:4], search='person')
    y = myactors[:4]

    myactors = df2[df2['department'] == 'Writing'].original_name.tolist()
    movie_writers, writers_url = search_person(
        myactors[:4], myactors[:4], search='person')
    z = myactors[:4]
...
def similarity2(movie, mylist=['genre', 'full_cast'], weight='balanced', pop=5, start_year=2000, end_year=2020, min_rating=5.0, Total_votes=100000):
    df = pd.read_csv(r'myfinaldata4.csv')
    my_movie, mymoviedate, myid, my_actor, my_director, my_writer, idx, mymovie, movie_image1, movie_genre, movie_production_companies, movie_cast, movie_directors, movie_writers, cast_url, directors_url, writers_url = movie_details(
        movie)
    ...
    df = df[((df.year >= start_year) & (df.year <= end_year)) & (
        df.votes >= Total_votes) & (df.avg_vote >= min_rating)]
    if df.empty:
        a = 0
        return a
    else:
        ...
        my_weight = (pop/10.0)
        df['metascore'] = df['metascore1'] * \
            my_weight + df['metascore2']*(1.0-my_weight)
        sorted_data = df.sort_values(by=['metascore'], ascending=False)
        name = sorted_data.iloc[:21, [1]].values.ravel().tolist()
        name = string_match(name, th=0.9)
        recommend_movie, recommend_url = search_movie(
            name, name, search='movie')
...
def recommendations():
    if flask.request.method == 'POST':
        ...
        myweight = [1.0]
        movie_name = m_name
        my_movie, mymoviedate, my_actor, trailer, my_director, my_writer, mymovie, movie_image1, movie_genre, movie_production_companies, movie_cast, movie_directors, movie_writers, cast_url, directors_url, writers_url, recommend_movie, recommend_url = similarity2(
            movie_name, mylist, myweight, popularity, start_date, end_date, rating, no_votes)
